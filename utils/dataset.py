...
from utils_train import letterbox_image
...
class ReadDataSet_random(Dataset):
    ...
    参数说明：
        ...
        img_dir：图片所在的文件夹路径
        transform：图片增强方法
        positive_rate：正例图片的选择概率，数值范围0~1。默认值：0.5
    """

    def __init__(self, data_csv, img_dir, transform=None, positive_rate=0.5):
        super().__init__()
        self.data = data_csv
    ...
    def __getitem__(self, idx):
        # 读取第一张图片和所属ID
        img_1 = cv.imread(self.img_dir + self.data.at[idx, 'img'])
        img_1 = cv.cvtColor(img_1, cv.COLOR_BGR2RGB)

        id_1 = self.data.at[idx, 'id']
        # 正例图片的概率
        p = np.random.rand()
        if p < self.pos_rate:
            n = np.random.choice(self.data[self.data['id'] == id_1].index, 1)[0]
        else:
            n = np.random.choice(self.data[self.data['id'] != id_1].index, 1)[0]

        # 读取第二张图片和所属ID
        img_2 = cv.imread(self.img_dir + self.data.at[n, 'img'])
        img_2 = cv.cvtColor(img_2, cv.COLOR_BGR2RGB)

        id_2 = self.data.at[n, 'id']
